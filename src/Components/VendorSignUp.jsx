...
export default function VendorSignupForm() {
  ...
  const handleSendOtp = async () => {
    ...
    setOtpLoading(true);
    try {
      await axios.post(`http://localhost:8080/otp/send?phoneNumber=${encodedPhoneNumber}`);
      setOtpSent(true);
    } catch (error) {
      console.error(error);
  ...
  const handleVerifyOtp = async () => {
    const mobileNumber = document.getElementById("mobile-number").value;
    try {
      await axios.post("http://localhost:8080/verifyOtp", {
        mobileNumber,
        otp,
      });
      setOtpVerified(true);
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100 p-6 mt-20">
      ...
      <div className="flex flex-col md:flex-row max-w-6xl bg-white rounded-lg shadow-lg overflow-hidden w-full">
        ...
        <div className="flex-1 relative md:wd-2/3">
          <img
            src="https://i.pinimg.com/736x/ac/7d/68/ac7d68f81d3d8e41b10f68b09de50b68.jpg"
            alt="Event Nexus Hub"
